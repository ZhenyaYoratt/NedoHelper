from PyQt5.QtWidgets import QMainWindow, QVBoxLayout, QLabel, QPushButton, QWidget, QMessageBox, qApp, QStyleFactory, QDialog, QComboBox
...
class SettingsWindow(QMainWindow, Window):
    ...
    def __init__(self, parent = None):
        ...
        self.setWindowTitle(make_title(qApp.tr("Настройки")))
        self.setFixedSize(320, 450)
        self.setWindowFlags(Qt.WindowType.Dialog)
        self.setWindowFlag(Qt.WindowType.WindowContextHelpButtonHint, False)

        self.statusbar = self.statusBar()
        self.statusbar.showMessage(qApp.tr("Готов к работе"))
        
        self.settings = Settings(self.SETTINGS_FILE, self)

        self._layout = QVBoxLayout()
        self.header_label = QLabel(qApp.tr("Настройки программы"))
        self.header_label.setObjectName("title")

        theme_layout = QVBoxLayout()
        self.theme_label = QLabel(qApp.tr("Тема"))
        self.theme_combobox = QComboBox()
        self.theme_combobox.addItems([qApp.tr("Системная тема"), qApp.tr("Темная тема"), qApp.tr("Светлая тема")])
        self.theme_combobox.currentIndexChanged.connect(self.change_theme)
        theme_layout.addWidget(self.theme_label)
        theme_layout.addWidget(self.theme_combobox)

        theme_style_layout = QVBoxLayout()
        self.theme_style_label = QLabel("Стиль темы")
        self.theme_style_combobox = QComboBox()
        self.theme_style_combobox.addItems([qApp.tr("Современная тема"), qApp.tr("Fusion тема"), qApp.tr("Плоская тема (Windows 10)"), qApp.tr("Windows 95 тема")])
        ...
        self.language_label = QLabel(qApp.tr("Язык"))
        ...
        self.save_button = QPushButton(qApp.tr("Сохранить"))
        ...
        if self.settings.filenotfound:
            dialog = QDialog(self)
            dialog.setWindowTitle(qApp.tr("Первоначальная настройка программы"))
            layout = QVBoxLayout()
            title = QLabel(qApp.tr("Добро пожаловать!"))
            title.setObjectName("title")
            layout.addWidget(title)
            layout.addWidget(QLabel(qApp.tr("Для начала работы необходимо настроить программу.")))
            layout.addWidget(QLabel(qApp.tr("Настройки будут сохранены в файл settings.json.")))
            layout.addLayout(theme_layout)
            layout.addLayout(theme_style_layout)
            layout.addLayout(language_layout)
            save_close_button = QPushButton(qApp.tr("Сохранить и закрыть"))
            save_close_button.clicked.connect(lambda: (self.save_settings(), dialog.close()))
    ...
    def apply_theme(self):
        ...
        if theme_style == "modern":
            ...
            self.parent().update_button_icons("light")
        self.statusbar.showMessage(qApp.tr("Тема успешно применена."))
    ...
    def load_settings(self):
        ...
        self.apply_theme()
        translator = QTranslator()
        translator.load("localizations/en.qm")
        qApp.installTranslator(translator)

    def save_settings(self):
        """Сохраняет настройки в файл."""
        self.settings.save_settings()
        self.statusbar.showMessage(qApp.tr("Настройки успешно сохранены."))

    def retranslateUi(self):
        self.setWindowTitle(make_title(qApp.tr("Настройки")))
        self.header_label.setText(qApp.tr("Настройки программы"))
        self.theme_combobox.setItemText(0, qApp.tr("Системная тема"))
        self.theme_combobox.setItemText(1, qApp.tr("Темная тема"))
        self.theme_combobox.setItemText(2, qApp.tr("Светлая тема"))
        self.theme_style_combobox.setItemText(0, qApp.tr("Современная тема"))
        self.theme_style_combobox.setItemText(1, qApp.tr("Fusion тема"))
        self.theme_style_combobox.setItemText(2, qApp.tr("Плоская тема (Windows 110)"))
        self.theme_style_combobox.setItemText(3, qApp.tr("Windows 95 тема"))
        self.save_button.setText(qApp.tr("Сохранить"))
